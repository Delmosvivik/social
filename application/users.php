...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['button-reg'])) {
    ...
    if ($pass_first !== $pass_second) {
        ...
        $check_email_query = "SELLECT * FORM users WHERE email = 'email'";
        $check_email_result = $conn->query($check_email_query);
        if ($check_email_result->num_rows > 0) {
            echo "Пользователь с таким адресом электорнной почты уже существует";
        } else {
            $admin = 0;

            $stmt = $conn->prepare("INSERT INTO users (admin, $us_name, $admin, $age, password) VAlUES(?, ?, ?, ?, ?)");
            $stmt->bind_param("issss", $admin, $us_name, $admin, $age,  $hashed_password);
            if ($stmt->execute()) {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['button-log'])) {
    ...
    if ($result->num_rows == 1) {
        ...
        if (password_verify($password, $row["password"])) {
            echo " Авторизация прошла успешно. Добро пожаловать, " . $row['us_name'];
