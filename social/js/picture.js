...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    const fileInput = document.getElementById("file-input"); const profilePicture = document.getElementById("profile-picture");
    ...
    updatePictureButton.addEventListener("click", function () { fileInput.click(); });
    // Обработчик события изменения в поле ввода файла
    fileInput.addEventListener("change", async function () {
        // Создание объекта FormData для отправки файла на сервер
        const formData = new FormData(); formData.append("action", "update");
        formData.append("image", fileInput.files[0]); try {
            // Загрузка профильной картинки на сервер и обновление ее на странице
            const response = await uploadProfilePicture(formData);
            if (response.startsWith("success")) { profilePicture.src = response.split("|")[1]; } location.reload();
